...
Tokens are the through-line of the whole pipeline. Each token is mapped to an [embeddings](/glossary/embeddings/)
vector on the way in; [attention](/glossary/attention/) is the mechanism by which tokens weigh each other; the
[transformer](/glossary/transformer/) chews on the whole sequence of them at once; and generation is just predicting
the next token over and over, with [temperature](/glossary/temperature/) deciding how boldly the model gambles among
