...
related = ["machine-learning", "parameters", "val-loss", "latent-space", "norm-placement"]
...
repeat millions of times and the error grinds toward a bottom. The size of each step is the
...
_Adam_ adapt the step size per parameter as they go. One thing worth keeping straight: gradient
descent only ever drives the _training_ loss down. Whether the model is genuinely learning
rather than memorizing is what [validation loss](/glossary/val-loss/) is there to catch. And the machinery doesn't have to
