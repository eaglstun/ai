...
How the app was wired up, and how to refresh/rebuild the token. The day-to-day skill
(posting, metrics) doesn't need any of this — it's here for token refresh and disaster
recovery.

## Current state (2026-06-08)

Wired and working. Credentials live in `inbox/threads.md` (gitignored) and the working token
is in `.env` as `THREADS_ACCESS_TOKEN` / `THREADS_USER_ID`.

| Thing              | Value                                                         |
| ------------------ | ------------------------------------------------------------- |
| Meta app ID        | `2073647747364138` (dashboard URLs, app roles)                |
| Threads app ID     | `1366007229076487` (OAuth `client_id` — the one the API uses) |
| Account / tester   | `@eaglstun.ai` (Threads Tester, invite accepted)              |
| Numeric user id    | `36713181624962007` (`THREADS_USER_ID`)                       |
| Account id         | `17841464637884633` (distinct; shows up in insights URLs)     |
| Redirect callback  | `https://ai.ericeaglstun.com/api/callback`                    |
| Uninstall / Delete | `https://ai.ericeaglstun.com/api/{uninstall,delete}`          |

> **Don't confuse the two app IDs** — the Threads app ID (`1366…`) is the OAuth client, not
> the Meta app ID (`2073…`). The Meta app secret in `inbox/threads.md` traveled through a chat
> transcript and the working token has too — **regenerate both** from the dashboard when
> convenient, and the scripts won't care (they read `.env`).
...
3. **App roles → Threads Testers → Add People → Threads Tester → `eaglstun.ai`.** Sends an
   invite (account must be **public**).
4. **Accept the invite in Threads:** Settings → Account → Website permissions → **Invites** →
   Accept `ai.ericeaglstun.com`.
5. **Settings → User Token Generator → Generate Access Token** for the tester → 60-day
   long-lived token. Paste into `.env` (in an editor, not a shell command that echoes it).
6. Fetch the numeric user id: `GET /me?fields=id,username&access_token=…` → `THREADS_USER_ID`.
...
dead, re-run the User Token Generator (steps 5–6 above) — no need to redo the app config.
...
2. **Code → short-lived token** (1 hr): `POST https://graph.threads.net/oauth/access_token`
   with `client_id`, `client_secret`, `grant_type=authorization_code`, `redirect_uri`, `code`.
3. **Short → long-lived** (60 days):
...
The token posts **as Eric** and is effectively a password. Keep it in `.env` (gitignored),
never in committed files, never echoed through a shell command (which lands in transcripts).
Both the token and the Meta app secret are revocable/regenerable from the dashboard in seconds.
